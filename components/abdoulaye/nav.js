...
export function Nav(props = {}) {
  ...
  header.innerHTML = `
    <a class="abdoulaye-nav__logo" href="${escapeHtml(home.href)}"${home.dataRoute ? " data-route" : ""} aria-label="Retour à l'accueil">AD</a>
    <nav class="abdoulaye-nav__links" aria-label="Navigation principale">
      ${links.map((link) => `<a href="${escapeHtml(link.href)}"${link.dataRoute ? " data-route" : ""} class="abdoulaye-nav__link">${escapeHtml(link.label)}</a>`).join("")}
    </nav>
