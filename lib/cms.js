...
export function createCmsClient({ baseUrl, token = null, timeout = 8000, retries = 2 } = {}) {
  ...
  function fetchCMS(endpoint, options = {}) {
  ...
  return {
    ...
    async create(resource, data) {
      const payload = await fetchCMS(`api/${resource}`, { method: "POST", body: { data }, retries: 0 });
      return flatten(payload);
    },

    async update(resource, data) {
      const payload = await fetchCMS(`api/${resource}`, { method: "PUT", body: { data }, retries: 0 });
      return flatten(payload);
    },
